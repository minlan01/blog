...
def get_stats(db: DBSession):
    ...
    post_count = db.scalar(select(func.count()).select_from(Post).where(Post.status == "published")) or 0
    ...
    total_views = db.scalar(select(func.coalesce(func.sum(Post.view_count), 0)).where(Post.status == "published")) or 0
    total_words = db.scalar(
        select(func.coalesce(func.sum(func.length(Post.content_markdown)), 0)).where(Post.status == "published")
    ) or 0
    ...
    cache_set("stats:all", result, 300)
    return result
